...
class CrudProduct(View):
	...
	def get(self, request, prod_id=None):
		if prod_id:
			...
			context={'prod':prod, 'cats':self.cats, 'title':self.title}
			#print(prods.category.all())
			return render(request, self.template, context)
		else:
			context={'cats':self.cats, 'title':self.title}
			return render(request, self.template, context)
	...
	def post(self, request, prod_id=None):
		...
		if prod_id:
			...
			msg='Data updated...'
			context={'cats':self.cats, 'prod':prod, 'msg':msg, 'title':self.title}
			return render(request, self.template, context)
		else:
			prod=models.Product(
							name=name,
			...
			msg='Data inserted...'
			context={'cats':self.cats, 'msg':msg, 'title':self.title}
			return render(request, self.template, context)
